...
class Gtfs {
    ...
    async add(obj: any) {
        ...
        return this.schemaSplice(50_000)
    }
...
/**
 * download GTFS files
 * @param url GTFS zip url
 * @param filePath path destination
 */
export const downloadFiles = async (url: string, filePath: string) => {
...
const importDocsWithEdges = async (gtfsName: NameDefinitions, edges: Edge[], docs: Schema[]) => {
    ...
    if (gtfsName === "pathways") {
        await pathwaysCollection.import(<Pathway[]>docs)
    }
    if (gtfsName === "calendar_dates") {
        await calendarDatesCollection.import(<CalendarDates[]>docs)
    }
    if (gtfsName === "agency") {
        await agencyCollection.import(<Agency[]>docs)
    }
    if (gtfsName === "calendar") {
        await calendarCollection.import(<Calendar[]>docs)
    }
}
...
/**
 * read and parse GTFS (csv) files
 * @param filePath GTFS folder path
 */
    ...
    for (const textFile of textFiles) {
        ...
        parser.on("readable", async () => {
            ...
            while ((record = parser.read()) !== null) {
                ...
                if (data) {
                    console.log(`${new Date().toISOString()} ${gtfs.name} ${data.docs.length} ${data.edges.length}`)
                    await importDocsWithEdges(gtfs.name, data.edges, data.docs)
                }
            }
        })
        parser.on("end", async () => {
            console.log(`${new Date().toISOString()} ${gtfs.name}`)
        ...
        stream.on("close", async () => {
            ...
            if (gtfsFiles.files.size === 0) {
                if(!textFiles.includes("calendar.txt")) {
                    await createEdgesServes(true)
                }
...
/**
 * generate precedes edges
 *
 * (stops)--precedes-->(stops)
 */
export const createEdgesPrecedes = async () => {
    ...
    try {
        ...
        `,
            { intermediateCommitCount: 50_000 }
        )
    } catch (error: any) {
...
/**
 * generate serves edges
 *
 * (calendar)--serves-->(trips)
 */
export const createEdgesServes = async (onlyDates: boolean = false) => {
...
/**
 * generate has_routes edges
 *
 * (stops)--has_routes-->(routes)
 */
export const createEdgesHasRoute = async () => {
    ...
    try {
        ...
        `,
            { intermediateCommitCount: 10_000 }
        )
    } catch (error: any) {
        console.log(error.message)
    }
